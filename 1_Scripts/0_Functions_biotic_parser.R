...
### File: 0_Functons_biotic_parser.R
### Time-stamp: <2020-07-04 06:43:26 a23579>
###
### Created: 04/07/2020	06:43:19
### Author: Yves Reecht
###
####################################################################################################
### Description:
...
dm <- list(KeyType="as.character", "CompositeTaxaKeyType"="as.character","CompositeTaxaSexKeyType"="as.character",
           "xs:integer"="as.integer", "xs:string"="as.character", "xs:decimal"="as.double", "key"="as.character",
           "xs:date"="as.Date", "xs:time"="as.character")
...
set_data_types <- function(bioticdata, schema, keys, foreign_keys, datatype_mapping=dm){
    get_data_types <- function(typename){
        ...
        xmltypename <- paste(toupper(substring(s, 1,1)), substring(s, 2),
                             sep="", collapse=" ")
        elements <- getNodeSet(schema,
                               paste("/xs:schema/xs:complexType[@name='",
                                     xmltypename,"']//xs:element", sep=""),
                               c(xs="http://www.w3.org/2001/XMLSchema"))
        attribs <- getNodeSet(schema,
                              paste("/xs:schema/xs:complexType[@name='",
                                    xmltypename,"']//xs:attribute", sep=""),
                              c(xs="http://www.w3.org/2001/XMLSchema"))
...
make_foreign_keys <- function(node, keys_table, foreign_keys_table, schematype_function){
    ...
    if (schematype!="MissionType"){
        foreign_keys <- append(make_foreign_keys(xmlParent(node), keys_table,
                                                 foreign_keys_table, schematype_function), foreign_keys)
    }
...
foreing_key_generator_3 <- function(node)
{
    return(make_foreign_keys(node, keys_biotic3, foreign_keys_biotic3, hardcoded_schematype_function))
}
